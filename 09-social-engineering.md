...
performing actions, or granting access to systems without realizing the security risks involved.
Instead of directly attacking systems, attackers exploit **human psychology** to bypass security
measures.
...
1. **Phishing** – Fake emails or websites trick users into giving credentials.
2. **Spear Phishing** – Targeted phishing attack on a specific person or organization.
3. **Whaling** – Phishing aimed specifically at senior executives.
4. **Vishing** – Voice phishing using phone calls.
5. **Smishing** – SMS-based phishing attacks.
6. **Pretexting** – Fabricating a scenario to get personal information.
7. **Baiting** – Offering something attractive (e.g., free USB drive) to trick users.
8. **Tailgating / Piggybacking** – Following authorized personnel into restricted areas.
9. **Quid Pro Quo** – Offering a service in exchange for confidential information.
10. **Reverse Social Engineering** – The attacker sets up a situation so the victim initiates
    contact and trusts the attacker as the "solution".
...
## 🧪 Hands-on Labs
- **Lab 1: Simulated Phishing Email Campaign** – Create a phishing email template, send to a test
  group, and track click rates.
- **Lab 2: USB Drop Baiting** – Load a harmless file onto USB drives and drop them in a controlled
  environment; measure how many are plugged in.
- **Lab 3: Vishing Simulation** – Call users pretending to be IT support; request harmless test
  information for awareness training.
...
- Use **mantraps/access vestibules** to prevent tailgating.
...
Written authorization (Rules of Engagement) must always be obtained before testing real employees.
